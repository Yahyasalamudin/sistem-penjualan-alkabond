...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    @if ($user->role == 'owner')
        ...
        <div class="col-2">
            <form action="{{ route('filter-city') }}" method="post" id="filterCityForm">
                @csrf
                <select class="form-control  @error('active') is-invalid @enderror" name="filter_city"
                    id="input-filter-kota">
                    <option value="">Semua Cabang</option>
                    @foreach ($cities as $city)
                        <option value="{{ $city->id }}" {{ session('filter-city') == $city->id ? 'selected' : '' }}>
                            {{ $city->city }}</option>
                    @endforeach
                </select>
            </form>
    ...
    <div class="col-2">
        <form action="{{ route('filter-city-branch') }}" method="post" id="filterCityBranchForm">
            @csrf
            <select class="form-control  @error('active') is-invalid @enderror" name="filter_city_branch"
                id="input-filter-cabang-kota">
                <option value="">Semua Kota</option>
                @foreach ($city_branches_topbar as $branch)
                    @if (
                        ($user->role === 'owner' && $branch->city_id == session('filter-city')) ||
                            ($user->role === 'admin' && $branch->city_id == $user->city_id))
                        <option value="{{ $branch->id }}"
                            {{ session('filter-city-branch') == $branch->id ? 'selected' : '' }}>
